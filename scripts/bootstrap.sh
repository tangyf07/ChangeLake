#!/usr/bin/env bash
# Download Paimon + shaded Hadoop + Paimon S3 + Flink CDC MySQL SQL connector + MySQL JDBC.
...
# Pre-bundled Hadoop jar required for filesystem ops outside a Hadoop cluster.
...
# S3/MinIO filesystem plugin (official Paimon 1.4 docs)
# Maven: org.apache.paimon:paimon-s3:1.4.2
# Docs: https://paimon.apache.org/docs/1.4/maintenance/filesystems/
PAIMON_S3_JAR="paimon-s3-${PAIMON_VERSION}.jar"
PAIMON_S3_URL="https://repo1.maven.org/maven2/org/apache/paimon/paimon-s3/${PAIMON_VERSION}/${PAIMON_S3_JAR}"
...
echo "[bootstrap] Paimon ${PAIMON_VERSION} for Flink ${FLINK_MINOR} (+ paimon-s3 for MinIO)"
...
echo "[bootstrap] S3/MinIO:    https://paimon.apache.org/docs/1.4/maintenance/filesystems/"
...
download "$PAIMON_S3_URL" "$LIB_DIR/$PAIMON_S3_JAR"
